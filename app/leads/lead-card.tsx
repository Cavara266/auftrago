import Link from "next/link";
import UnlockButton from "@/app/leads/unlock-button";

type LeadCardProps = {
  lead: {
    id: string;
    title: string;
    category: string;
    city: string;
    description: string;
    priceCredits: number;
    isUnlocked: boolean;
  };
};

export default function LeadCard({ lead }: LeadCardProps) {
  return (
    <div className="rounded-3xl border border-white/10 bg-black/20 p-6">
      <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="mb-4 flex flex-wrap gap-2">
            <span className="rounded-full bg-white/10 px-3 py-1 text-sm text-white/70">
              Neu
            </span>

            <span className="rounded-full bg-white/10 px-3 py-1 text-sm text-white/70">
              {lead.category}
            </span>

            <span className="rounded-full bg-white/10 px-3 py-1 text-sm text-white/70">
              {lead.city}
            </span>

            <span
              className={`rounded-full px-3 py-1 text-sm ${
                lead.isUnlocked
                  ? "bg-emerald-500/20 text-emerald-200"
                  : "bg-amber-500/20 text-amber-200"
              }`}
            >
              {lead.isUnlocked ? "Kontakt freigeschaltet" : "Kontakt gesperrt"}
            </span>
          </div>

          <h2 className="text-3xl font-semibold text-white">
            {lead.category} {lead.city}
          </h2>

          <div className="mt-4 flex flex-wrap gap-3 text-sm text-white/60">
            <span>📍 {lead.city}</span>
            <span>🔒 Details nach dem Öffnen</span>
            <span>💳 {lead.priceCredits} Credits</span>
          </div>
        </div>

        <div className="flex shrink-0 flex-col gap-3">
          <Link
            href={`/leads/${lead.id}`}
            className="inline-flex items-center justify-center rounded-full border border-white/10 px-5 py-3 text-sm font-semibold text-white hover:bg-white/10"
          >
            Details ansehen
          </Link>

          {!lead.isUnlocked ? (
            <UnlockButton
              leadId={lead.id}
              priceCredits={lead.priceCredits}
              isUnlocked={lead.isUnlocked}
            />
          ) : null}
        </div>
      </div>

      <div className="mt-6 text-2xl font-bold text-white">
        {lead.priceCredits} Credits
      </div>
    </div>
  );
}